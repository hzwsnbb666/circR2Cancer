...
def predictCircRNAToCancer(cancerNum,circRNANum,circRNACancerData,circRNAName):
    ...
    resultList = []
    for i in range(cancerNum):
        resultList.append((cancerData[i][1], np.around(resultMatrix[circRNAId - 1][circRNANum+i], decimals=9)))
...
def predictCancerToCircRNA(cancerNum,circRNANum,circRNACancerData,cancerName):
    resultMatrix, ROC_AUC, Mean_TPR, Mean_FPR = tf.AnalyseAlgrithom('KATZHCDA')
    ...
    resultList = []
    for i in range(circRNANum):
        resultList.append((circRNAData[i][1], np.around(resultMatrix[i][cancerId - 1+circRNANum], decimals=9)))
    resultList.sort(key=lambda x:x[1],reverse=True)
    return resultList
